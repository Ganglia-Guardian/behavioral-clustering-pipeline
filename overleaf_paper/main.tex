...
Random-sampled AP is currently the most reliable replacement for full AP: it runs in under a minute on all tested datasets and achieves ARI $\geq 0.425$ on the three datasets where full AP is available. K-means and stratified sampling often match or slightly exceed random sampling in ARI---on \texttt{comparison\_test}, k-means reaches 0.458 versus 0.425 for random---so they are worth considering when sample composition matters.
...
Sparse AP consistently over-fragments. The cluster counts are 3--5$\times$ higher than full AP across nearly all datasets, and this is not easily fixed by tuning---it is a consequence of the graph structure (Section~\ref{sec:clustering-methods}). It is retained for completeness but is not recommended for production use.

HDBSCAN occasionally achieves higher silhouette than any AP variant (e.g.\ \texttt{moving\_test}: 0.580), but its ARI against full AP is essentially zero, and it assigns 50--65\% of windows as noise on high-activity datasets. It reflects a genuinely different objective and should be treated as a separate exploratory tool rather than a drop-in replacement.

\section{Discussion}

\subsection{Recommended Current Method}

Random-sampled AP is the current recommended default for replacing the MATLAB pipeline. It runs in 18--51\,s on all tested datasets and stays close to full AP where comparison is possible. For applications where rare behavioral motifs---tremor, impaired gait---are scientifically important, stratified or coreset sampling is preferable: both ensure that small behavioral clusters are represented in the AP sample regardless of how infrequently they occur. K-means sampling is a reasonable middle ground that selects cluster-central rather than boundary points and showed the best ARI on \texttt{comparison\_test}.

Full AP remains the ground-truth reference and should be run whenever $N$ is small enough (under $\sim$20,000 windows on a machine with 12+ GB RAM).
...
The pipeline matches the MATLAB AP output on small datasets ($N \leq 20{,}000$) and extends cleanly to larger recordings through subsampling. Of the scalable variants, random-sampled AP is the most battle-tested; k-means and stratified sampling are newer additions that look competitive and warrant more evaluation as more sessions are processed. The next priority is running timing benchmarks for those two methods and testing all variants on MitoPark longitudinal data, where the biological question---whether cluster usage shifts with disease progression---can be posed directly.
